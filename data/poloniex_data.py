"""
getting poloniex data python code originally written by
https://github.com/jyunfan
Editted by HY Kim, smilekhy88@gmail.com
"""
...
INTERVAL = 1800         # set chart interval (sec)
FETCH_URL = "https://poloniex.com/public?command=returnChartData&currencyPair=%s&start=%d&end=%d&period=%d"
DATA_DIR = "data"
COLUMNS = ["datetime","high","low","open","close","volume","quoteVolume","weightedAverage"]
...
def get_data(pair, interval):
    ...
    url = FETCH_URL % (pair, start_time, end_time, interval)
    ...
    if newfile:
        df.to_csv(outf, columns=COLUMNS, index=False)
    else:
        df.to_csv(outf, columns=COLUMNS, header=False, index=False)
    outf.close()
...
def main():
    ...
    for pair in pairs:
        get_data(pair, interval=INTERVAL)
        time.sleep(2)
